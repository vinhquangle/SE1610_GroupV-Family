...
<<<<<<< HEAD
    ...
    private static final String SEARCH_CUSTOMER = "SELECT    customerID, Name, [Password], Email, [Address], Phone, Point, [Status], [Delete]\n"
            + "FROM         tblCustomer\n"
            + "WHERE customeprivate static final String UPDATE = \"UPDATE tblCustomer SET name=?, password=?, email=?, [address]=?, phone=?, point=?, [status]=?, [delete]=? WHERE customerID=?\";rID LIKE ?";
